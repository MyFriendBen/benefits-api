...
*All scenarios use 2025 claim-year rules (income limit $43,389; refund table from the 2025 booklet). Expense-object key names mirror `income_streams` and need dev confirmation at import. County naming convention confirmed 2026-07-08: bare county names, no "County" suffix (e.g., `Sedgwick`, not `Sedgwick County`) — no KS white-label file exists yet to check against directly, but the sibling KS TANF ticket (MFB-1062) already resolved this exact question ("Confirmed name-only, no 'County' suffix") and all 20 scenarios below are already consistent with it.*
