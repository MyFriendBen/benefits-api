...
class TestUnreadableProgramsAreDropped(PeInputTestBase):
    ...
    def test_dropped_when_the_model_predates_their_output(self):
        ...
        with patch("integrations.clients.policyengine.policy_engine.capture_message"):
    ...
    def test_reports_the_drop(self):
        ...
        with patch("integrations.clients.policyengine.policy_engine.capture_message") as capture:
...
class TestResolveUnpinnedVersion(TestCase):
    ...
    def _mock_get(self, *, json_data=None, exc=None, status_ok=True):
        ...
        if exc is not None:
            return patch(
                "integrations.clients.policyengine.versions.requests.get",
                side_effect=exc,
            )
        return patch(
            "integrations.clients.policyengine.versions.requests.get",
