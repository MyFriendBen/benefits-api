...
4. **Household resources/assets must not exceed $12,000**
   ...
   - **SSI recipient present:** no countable resource figure is reported, so the resource test does not deny the household. WAC 388-470-0045 excludes an SSI recipient's resources, and `household_assets` is a single figure with no per-member ownership to isolate their share from. Read from a reported `sSI` amount or the SSI current-benefit tile. Deliberately inclusive: such a household passes the resource test whatever the reported total. PE's own `wa_tanf_countable_resources` does not model the exclusion at all (MFB-1696).
