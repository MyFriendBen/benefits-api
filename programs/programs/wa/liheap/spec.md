...
1. **Household income must be at or below 150% FPG (heating/crisis)**
   ...
   - Note: TANF, SSI, and SNAP income counts toward gross income but does not confer automatic eligibility. WA does not implement categorical eligibility (WA 2026 Model Plan §1.4). Do not use `has_benefit("tanf")`, `has_benefit("snap")`, or `has_benefit("ssi")` as eligibility shortcuts.
...
5. **Tribal households: 60% SMI income threshold; categorical eligibility available at 12 of 19 tribal grantees** ⚠️ *pending field request*
   ...
   **Tribal categorical eligibility:**
   ...
   - Screener fields: `tribal_affiliation` *(pending)*, `has_benefit("tanf")`, `has_benefit("ssi")`, `has_benefit("snap")`, `va_pension` current benefit *(pending)*, `household_size`, `calc_gross_income("yearly", ["all"])` (for annual income comparison against the 60% SMI threshold table)
   - Suggestions:
