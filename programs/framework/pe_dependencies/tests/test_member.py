...
class TestNonSsiBankAccountAssetsDependency(TestCase):
    """Tests for NonSsiBankAccountAssetsDependency, the person-level half of
    PolicyEngine's `spm_unit_cash_assets`. The household's reported liquid assets are
    attributed only to members who do not report SSI, so PolicyEngine's SSI-resource
    exclusion has person-level values to subtract."""
    ...
    def _add(self, ssi_amount=0, **kwargs):
        m = HouseholdMember.objects.create(
            screen=self.screen, relationship=kwargs.pop("relationship", "headOfHousehold"), age=40, **kwargs
        )
        if ssi_amount:
            IncomeStream.objects.create(
                screen=self.screen, household_member=m, type="sSI", amount=ssi_amount, frequency="monthly"
            )
        return m

    def _dep(self, m):
        return member.NonSsiBankAccountAssetsDependency(self.screen, m, {})

    def test_field_name(self):
        self.assertEqual(self._dep(self._add()).field, "bank_account_assets")

    def test_no_ssi_member_splits_the_aggregate_and_preserves_the_total(self):
        """With nobody on SSI this must reproduce spm_unit_cash_assets exactly, so the
        dependency is a no-op for every household the exclusion does not concern."""
        a = self._add()
        b = self._add(relationship="spouse")
        self.assertEqual(self._dep(a).value() + self._dep(b).value(), 1500)

    def test_ssi_member_gets_zero(self):
        ssi_member = self._add(ssi_amount=750)
        self.assertEqual(self._dep(ssi_member).value(), 0)

    def test_ssi_member_share_goes_to_the_non_ssi_member(self):
        """The whole aggregate lands on the non-SSI member: none of it is attributed to
        the member whose resources Missouri excludes."""
        ssi_member = self._add(ssi_amount=750)
        other = self._add(relationship="child")
        self.assertEqual(self._dep(ssi_member).value(), 0)
        self.assertEqual(self._dep(other).value(), 1500)

    def test_every_member_on_ssi_returns_zero(self):
        a = self._add(ssi_amount=750)
        b = self._add(relationship="spouse", ssi_amount=750)
        self.assertEqual(self._dep(a).value(), 0)
        self.assertEqual(self._dep(b).value(), 0)

    def test_no_assets_reported(self):
