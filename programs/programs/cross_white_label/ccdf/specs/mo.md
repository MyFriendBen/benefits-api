...
One uncaught error path is reachable rather than merely unexercised, and no scenario asserts it: an
`Expense` whose `frequency` is null raises rather than falling open, and `Expense.missing_fields()`
does not require `frequency`, so the state survives a complete screen. It is recorded as a
screener-data finding in review-notes.
