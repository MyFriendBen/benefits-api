...
class TxHcv(ProgramCalculator):
    ...
    def _count_dependents(self) -> int:
        ...
        for member in self.screen.household_members.all():
            ...
            if age is None:
                continue
            under_18 = age < 18
            adult_dependent = age >= 18 and (member.has_disability() or member.student_full_time)
    ...
    def _is_elderly_or_disabled_family(self) -> bool:
        ...
        for member in self.screen.household_members.all():
            if member.relationship in self._elderly_family_relationships:
                age = member.calc_age()
                if (age is not None and age >= 62) or member.has_disability():
                    return True
        return False
    ...
    def _head_age_ok(self) -> bool:
        ...
        head_age = head.calc_age() if head is not None else None
        if head_age is None:
            return True
        return head_age >= self.min_head_age
