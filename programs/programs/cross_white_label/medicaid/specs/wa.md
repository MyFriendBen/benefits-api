...
13. **Foster Care, Adoption Support, and Foster Care Alumni: Federal Medicaid categorical pathways; no income test**
    ...
    * **Sub-data-gap (Alumni only) — closed.** `household_member.was_in_foster_care` (Step 5, "Ever in foster care, even briefly") now captures the foster-care history fact. PolicyEngine's `medicaid_work_requirement_eligible` applies the age window itself (`was_in_foster_care & (age < former_foster_care_age_limit)`), so no age logic is needed here. ⚠️ WA Medicaid does not declare `FosterCareDependency` itself — the value only reaches PolicyEngine because Head Start declares it and every program shares one merged household payload per screen. Deactivate WA Head Start and this pathway silently stops working; adding `FosterCareDependency` to the Medicaid `pe_inputs` explicitly is the durable fix. Retain the alumni pathway in the program description as a backstop for users who don't tick the tile, and note the tile does not capture whether the user was *on Medicaid* at 18, which the federal rule also requires.
