...
# Income types collected so the reported dollar amount can be sent to PolicyEngine as that
# benefit's own input (the `snap`/`wic` fields), but which are NOT countable income: federal
# rules exclude SNAP and WIC from income tests, and PolicyEngine computes both separately, so
# folding them into an aggregate would double-count. calc_gross_income() therefore skips them
# for the "all"/"earned"/"unearned" buckets and returns them only when named explicitly, e.g.
# calc_gross_income("yearly", ["snap"]).
