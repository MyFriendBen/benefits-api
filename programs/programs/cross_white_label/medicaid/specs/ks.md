...
No ordering change is needed. `CALC_ORDER` in `screener/views.py` derives the Medicaid block from `Medicaid.__subclasses__()`, so a calculator inheriting the family base is ordered ahead of the programs gating on it as soon as it exists.
