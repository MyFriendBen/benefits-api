...
class TestTrumpAccountRegistration(TestCase):
    ...
    def test_registered_in_federal_calculators(self) -> None:
...
class TestTrumpAccountPilotWindow(TestCase):
    ...
    def test_start_boundary_jan_2025_is_eligible(self) -> None:
        self.assertTrue(self._run_member_eligible(date(2025, 1, 1)))

    def test_end_boundary_dec_2028_is_eligible(self) -> None:
        self.assertTrue(self._run_member_eligible(date(2028, 12, 1)))

    def test_mid_window_2026_is_eligible(self) -> None:
        self.assertTrue(self._run_member_eligible(date(2026, 6, 1)))

    def test_one_month_before_window_dec_2024_is_ineligible(self) -> None:
        self.assertFalse(self._run_member_eligible(date(2024, 12, 1)))

    def test_one_month_after_window_jan_2029_is_ineligible(self) -> None:
        self.assertFalse(self._run_member_eligible(date(2029, 1, 1)))

    def test_birth_year_month_none_is_ineligible(self) -> None:
...
class TestTrumpAccountAgeCeiling(TestCase):
    ...
    def test_age_0_is_eligible(self) -> None:
        self.assertTrue(self._run_member_eligible(0))

    def test_age_17_is_eligible(self) -> None:
        self.assertTrue(self._run_member_eligible(17))

    def test_age_18_is_ineligible(self) -> None:
        self.assertFalse(self._run_member_eligible(18))

    def test_age_19_is_ineligible(self) -> None:
...
class TestTrumpAccountPregnancy(TestCase):
    ...
    def test_due_date_inside_pilot_window_is_eligible(self) -> None:
    ...
    def test_due_date_after_pilot_window_is_ineligible(self) -> None:
        # reference_date such that due_date falls after Dec 2028
        reference_date = date(2029, 1, 1)
        self.assertFalse(self._run_member_eligible(reference_date))

    def test_pregnant_member_skips_birth_year_month_check(self) -> None:
...
class TestTrumpAccountValue(TestCase):
    ...
    def test_two_eligible_members_each_receive_1000(self) -> None:
        calculator = make_calculator()
        members = [
    ...
    def test_mixed_household_only_eligible_members_receive_value(self) -> None:
        calculator = make_calculator()
