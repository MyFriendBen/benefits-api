...
5. **Individual child is categorically eligible for free meals through Head Start, Early Head Start, foster care, homelessness, migrant status, or runaway status** ⚠️ *partial data gap*
   ...
   - ⚠️ **Accepted over-widening — whole-unit spillover from any member's foster history.** We do not send `was_in_foster_care` for this program, but PolicyEngine receives one merged household payload per screen, so the value Head Start supplies is visible here too. PolicyEngine puts `was_in_foster_care` in the **SPMUnit-level** `gov.usda.school_meals.categorical_eligibility` adds-list, so *any one* member's flag makes the *whole unit* categorically free-meal eligible — including a child whose only qualifying link is an adult's foster-care history. This is the household-wide overextension the note above warns against, accepted deliberately: it runs in the inclusivity direction MFB applies throughout, the school verifies status at application, and the narrower alternative (age-scoping the flag) would cost the former-foster-youth Medicaid and CO EITC pathways that need the adult signal. The pre-existing `fosterChild` relationship already produced the same spillover onto that child's siblings; the tile widens it to adults.
