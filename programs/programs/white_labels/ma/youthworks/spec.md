...
- **Notes**: MFB does not collect risk/demographic factor information. Given the breadth of the 11 qualifying factors and the program's focus on low-income MA youth, the vast majority of users meeting criteria 1–3 will meet this criterion. Inclusivity assumption applied: all users meeting age, income, and residency are assumed to qualify here. Surfaced in the description using "underserved communities or who face barriers to employment" with named examples (housing instability, limited English, disability, justice involvement) — broad enough to cover LGBTQ+ youth and youth of color without naming sensitive identity categories explicitly.
  - **Optional screener improvements:**
    ...
    - **Shipped:** the "Foster Care" Special Circumstances tile now exists as `household_member.was_in_foster_care` (boolean, per-member) — "Ever in foster care, even briefly". More complete than `relationship: "fosterChild"`, which only captures foster children listed as household members and misses youth who are HoH and in/aged out of foster care. YouthWorks is a custom calculator and does **not** read it yet: wiring it in would narrow criterion 4 from the current blanket inclusivity assumption to an actual check, which is a deliberate eligibility change and needs its own ticket. Note it can only ever *widen* confidence here, never gate — foster care is one of eleven qualifying risk factors, so a `false` says nothing.
