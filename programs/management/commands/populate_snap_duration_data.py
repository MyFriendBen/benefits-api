    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            if not snap_programs.exists():
                self.stdout.write(
                    self.style.WARNING(f'No SNAP programs found for white label: {white_label_code}')
                )
                return
        ...
        if not dry_run:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully processed SNAP duration data: "
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Dry run completed: would create {created_count}, would update {updated_count}"
                )
            )
