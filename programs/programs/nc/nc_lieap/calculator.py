...
class NCLieap(ProgramCalculator):
    ...
    def household_eligible(self, e: Eligibility):
        ...
        earned_deduction = (
            0
            if getattr(self, "_has_senior_or_disabled", False)
                else  NCLieap.earned_deduction 
            )
        ...
        fpl_percent = (
            self.fpl_percent_senior_disabled
            if getattr(self, "_has_senior_or_disabled", False)
            else self.fpl_percent
        )

        #Set medical_deduction  based on senior/disabled status
        medical_deduction = (
        NCLieap.medical_deduction * 12
        if getattr(self, "_has_senior_or_disabled", False)
        else 0
        )
