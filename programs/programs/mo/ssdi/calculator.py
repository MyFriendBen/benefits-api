...
class MoSsdi(ProgramCalculator):
    ...
    # SSA estimated average monthly benefit for all disabled workers payable
    # January 2026, after the 2.8% COLA.
    member_amount = 1_630 * 12
    ...
    def _get_fra(birth_year: int) -> tuple[int, int]:
        ...
        if birth_year >= 1960:
            return (67, 0)
        for max_year, years, months in MoSsdi.FRA_SCHEDULE:
    ...
    def _is_under_fra(birth_year: int, birth_month: Optional[int], reference_date: date) -> bool:
        fra_years, fra_months = MoSsdi._get_fra(birth_year)
