...
| 2026-04-28 | cdadams | Flipped `has_calculator: true` → `has_calculator: false` in `wa_ssi_initial_config.json` (CodeRabbit critical lint). The previous value was incorrect for a discovery-only PR — `programs/programs/wa/__init__.py` defines an empty `wa_calculators` dict and no `wa_ssi` calculator is registered in any registry (PE, SPM, or `wa_calculators`). Importing with `has_calculator=true` would persist the flag to the DB but the screener would fail to instantiate a calculator at runtime. This matches the established **tracking-program pattern** in migrations `0137_create_tracking_programs.py`, `0138_backfill_has_calculator_base_program.py`, and `0141_create_gap_tracking_programs.py`, which all set `has_calculator=False` when no calculator code exists. The follow-up implementation PR will (a) add the `WaSsi` calculator class, (b) register it in `wa_calculators`, and (c) flip this flag back to `true` in the same atomic change. |
