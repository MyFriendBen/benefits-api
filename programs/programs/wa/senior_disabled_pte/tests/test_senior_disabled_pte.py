...
class TestMemberEligibility(TestCase):
    ...
    # Age pathway
    def test_age_67_eligible(self) -> None:
        self.assertTrue(self._run(make_member(age=67)))

    def test_age_61_boundary_eligible(self) -> None:
        self.assertTrue(self._run(make_member(age=61)))

    def test_age_60_ineligible(self) -> None:
...
class TestHouseholdEligibility(TestCase):
    ...
    def test_king_county_below_threshold_eligible(self) -> None:
        self.assertTrue(self._run("King County", 14_400))

    def test_king_county_at_threshold_3_eligible(self) -> None:
        self.assertTrue(self._run("King County", 84_000))

    def test_king_county_above_threshold_3_ineligible(self) -> None:
        self.assertFalse(self._run("King County", 84_001))

    def test_spokane_county_below_threshold_eligible(self) -> None:
        self.assertTrue(self._run("Spokane County", 30_000))

    def test_spokane_county_above_threshold_3_ineligible(self) -> None:
        self.assertFalse(self._run("Spokane County", 50_001))

    def test_unknown_county_uses_default_threshold(self) -> None:
...
class TestEndToEnd(TestCase):
    ...
    def test_eligible_senior_golden_path(self) -> None:
    ...
    def test_ineligible_age_60_no_pathway(self) -> None:
        calc = self._make_full_calc([make_member(age=60)], household_income=14_400)
        result = calc.calc()
        self.assertFalse(result.eligible)

    def test_eligible_veteran_with_disability(self) -> None:
    ...
    def test_ineligible_income_above_threshold(self) -> None:
        calc = self._make_full_calc([make_member(age=68)], household_income=85_000)
        result = calc.calc()
        self.assertFalse(result.eligible)

    def test_eligible_multi_member_household(self) -> None:
