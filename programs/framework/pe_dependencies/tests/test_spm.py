...
class TestCashAssetsExcludingSsiHouseholdsDependency(TestCase):
    """Reports the household's liquid assets, or nothing when a member's resources are
    excluded from the program's resource test and their share cannot be isolated."""

    def setUp(self):
        self.white_label = WhiteLabel.objects.create(name="Test State", code="test_ssiassets", state_code="TS")
        self.screen = Screen.objects.create(
            white_label=self.white_label,
            zipcode="63101",
            household_size=2,
    ...
    def _value(self):
        return spm.CashAssetsExcludingSsiHouseholdsDependency(self.screen, None, {}).value()

    def test_field_matches_the_plain_aggregate(self):
        self.assertEqual(spm.CashAssetsExcludingSsiHouseholdsDependency.field, "spm_unit_cash_assets")

    def test_reports_assets_when_no_ssi(self):
        self.assertEqual(self._value(), 1500)

    def test_reports_nothing_when_a_member_has_an_ssi_amount(self):
        IncomeStream.objects.create(
            screen=self.screen, household_member=self.head, type="sSI", amount=900, frequency="monthly"
        )
        self.assertEqual(self._value(), 0)

    def test_reports_nothing_when_the_ssi_amount_is_a_child_s(self):
        IncomeStream.objects.create(
            screen=self.screen, household_member=self.child, type="sSI", amount=750, frequency="monthly"
        )
        self.assertEqual(self._value(), 0)

    def test_no_assets_reported_is_still_zero(self):
        self.screen.household_assets = 0
        self.screen.save()
        self.assertEqual(self._value(), 0)

    def test_reports_nothing_when_the_ssi_tile_is_ticked_with_no_amount(self):
        """The tile establishes that somebody's resources are excluded even though it
        identifies nobody — which is exactly the case we cannot decompose."""
        seed_program(self.white_label, "ssi", base_program="ssi")
        _write_current_benefits(self.screen, ["ssi"])
