...
class TestNCHeadStart(TestCase):
    ...
    def create_household_member(self, screen, relationship="child", age=4, pregnant=False, disabled=False, 
                                has_income=False, insurance_type=None, birth_year=None, birth_month=None):
    ...
    def test_case_1_pregnant_with_ssi_eligible(self, mock_fetch):
        ...
        self.assertEqual(eligibility.value, 827 * 12, 
                        "Estimated value should be $9,924 (infant rate × 12)")
    ...
    def test_case_2_over_income_not_eligible(self, mock_fetch):
        ...
        self.assertFalse(eligibility.eligible, 
                        "Should NOT be eligible - income exceeds 130% FPL without housing adjustment")
    ...
    def test_case_4_over_income_high_rent_eligible(self, mock_fetch):
        ...
        self.assertTrue(eligibility.eligible,
                       "Should be ELIGIBLE - housing cost adjustment brings income below 130% FPL")
        ...
        self.assertEqual(len(eligible_children), 1,
                        "Should have 1 eligible child (3-year-old)")
        
        # Value should be preschool rate * 12
        # Durham County preschool rate: $1,167/month (1000 in MARKET_RATES_DATA)
        self.assertEqual(eligibility.value, 1167 * 12,
                        "Estimated value should be $12,000 (preschool rate × 12)")
    ...
    def test_child_age_0_to_5_eligible(self, mock_fetch):
        ...
        for age, age_category in ages_to_test.items():
            with self.subTest(age=age):
                ...
                self.assertTrue(len(eligibility.eligible_members) > 0,
                              f"Child age {age} should be eligible")
    ...
    def test_child_age_6_to_17_with_disability_eligible(self, mock_fetch):
        ...
        # Parent
        parent = self.create_household_member(
            screen=screen,
            relationship="headOfHousehold",
            age=30
        )
    ...
    def test_child_age_6_to_17_without_disability_not_eligible(self, mock_fetch):
        ...
        eligible_children = [m for m in eligibility.eligible_members if m.member.relationship == "child"]
        self.assertEqual(len(eligible_children), 0,
                        "12-year-old without disability should NOT be eligible")
    ...
    def test_pregnant_member_eligible(self, mock_fetch):
        ...
        self.assertEqual(len(eligibility.eligible_members), 1,
                        "Pregnant person should be eligible (uses infant rate)")
    ...
    def test_multiple_children_different_ages(self, mock_fetch):
        ...
        eligible_children = [m for m in eligibility.eligible_members if m.member.relationship == "child"]
        self.assertEqual(len(eligible_children), 3,
                        "Should have 3 eligible children")
        self.assertEqual(eligibility.value, expected_value,
                        f"Estimated value should be ${expected_value}")
    ...
    def test_countable_income_types_only(self, mock_fetch):
        ...
        self.assertTrue(eligibility.eligible,
                       "Should be eligible - only wages ($2,000/month) are counted, child support ignored")
    ...
    def test_member_value_returns_zero(self, mock_fetch):
        ...
        self.assertEqual(calculator.member_value(child), 0,
                        "member_value should return 0 (value calculated at household level)")
