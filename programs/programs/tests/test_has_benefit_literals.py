...
1. **A base program name.** No white label ships a program literally named "snap" /
   "tanf" / "section_8" — they ship co_snap, ks_snap, wa_snap, … So `has_benefit("snap")`
...
2. **A name held in a list.** Every one of those calculators reached the bad name by
   ...
   production was broken. Two CESN energy calculators were still dead after the first pass
   for exactly this reason: their lists carried CO names (`leap`, `cowap`, `rtdlive`,
   `co_care`) that match nothing on a CESN screen.
