...
3. **Add `ks_medicaid` to `CALC_ORDER`** in `screener/views.py`, ahead of the programs that gate on it. A calculator gating on Medicaid calls `self.program_eligible("ks_medicaid")`, which raises `DependencyError` when Medicaid has not been calculated yet — so without the ordering, the KS ACA Premium Tax Credit and the KS Medicaid sub-pathways are omitted from results rather than answered.
