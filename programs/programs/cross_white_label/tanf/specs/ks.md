...
4. **Countable household assets must be below $3,000** ⚠️ *partial data gap*
   ...
   - **SSI recipient present:** no countable resource figure is reported, so the resource test does not deny the household. KEESM 2210 excludes an SSI recipient's resources, and `household_assets` is a single figure with no per-member ownership to isolate their share from. Read from a reported `sSI` amount or the SSI current-benefit tile. Deliberately inclusive: such a household passes the resource test whatever the reported total. PE's own `ks_tanf_resources_eligible` does not model the exclusion at all (MFB-1696).
