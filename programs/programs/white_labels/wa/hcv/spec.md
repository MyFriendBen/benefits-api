...
11. **A student at an institution of higher education who does not live with their parents is subject to a special eligibility rule.** ⚠️ *data gap (partial)*
    ...
    - Note: With the `relationship` field, several exemptions become evaluable:
      ...
      Remaining gaps: veteran status (partial proxy only), graduate/professional-student status, Vulnerable Youth status, and parental income for the dual-income fallback. The head's-own-past-foster-care gap is now **closable** — see the foster-youth enhancement note below.
    ...
    - **Screener enhancement (foster youth) — field now exists, calculator not yet wired.** `household_member.was_in_foster_care` ("Ever in foster care, even briefly") ships as the per-member past-foster-care Boolean this spec asked for, distinct from `relationship: fosterChild` (which identifies a child *in the household* who is currently a foster child). It closes the head's-own-foster-history half of the HUD "independent student" exemption. **WA HCV is a custom calculator and does not read it yet** — wiring it in *widens* eligibility (one more exemption satisfied, so fewer students flagged ineligible), which is a deliberate eligibility change and needs its own ticket.
      - ⚠️ Still not covered by that field, even once wired: the other independent-student routes — orphan, ward of court, emancipated minor, legal guardianship. A youth who was a ward of court but never in foster care still won't match.
      - Unrelated to the separate foster gap in the income rules below (foster placement vs. kinship care), which this field does **not** address. See `programs/programs/FOSTER_CARE_SCREENER_GAPS.md`.
