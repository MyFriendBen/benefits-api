...
12. **Foster care (current + former foster youth to age 26)** — both are mandatory federal categories where income is irrelevant, and in non-expansion KS they would otherwise fall through as "childless adults." Screener fields: `household_member.was_in_foster_care` (Step 5, "Ever in foster care, even briefly") and `household_member.relationship`. Handling: **largely closed.** The field ships as a per-member boolean; PolicyEngine's `medicaid_work_requirement_eligible` pairs it with `age` internally (`was_in_foster_care & (age < former_foster_care_age_limit)`) to exempt former foster youth from the work requirement, so no age logic is needed on our side. ⚠️ KS Medicaid does not declare `FosterCareDependency` itself — the value only reaches PolicyEngine because Head Start declares it and every program shares one merged household payload per screen. That coupling is invisible from this calculator: deactivate KS Head Start and this pathway silently stops working. Adding `FosterCareDependency` to the Medicaid `pe_inputs` explicitly is the durable fix and belongs to the Medicaid ticket, not the field ticket. Keep the `description` surfacing as a backstop either way.

13. **Adoption-assistance children** ⚠️ *data gap* — a mandatory federal IV-E category named on the KanCare eligibility page alongside foster children; most also qualify via the income-based child pathways (criteria 3–5), so the false-negative risk is smaller. Screener fields: none. Handling: **still open.** The shipped tile reads "Ever in foster care, even briefly" and deliberately does not mention adoption support, so an adoptee who doesn't consider themselves foster-placed won't tick it. Widening the tile copy was considered and rejected for this pass — it would blur the one question five foster-care programs gate on. `description` surfacing remains the handling.
...
3. **Add `ks_medicaid` to `STATE_MEDICAID_OPTIONS`** in `programs/framework/helpers.py`. The `medicaid_eligible()` helper gates other programs on calculated Medicaid eligibility — without KS, this program won't exclude Medicaid-eligible Kansans from the KS ACA Premium Tax Credit or feed KS Medicaid sub-pathways.
