...
10. **At least one household member must be a U.S. citizen or qualified non-citizen** ⚠️ *not screened — by design*
    - Note: This is a federal eligibility rule, but **MFB intentionally does not screen or gate on citizenship.** The calculation always runs as `CITIZEN`, so SNAP is computed and shown to everyone; status only affects which program cards are *displayed*, via the results-page `legal_status_required` filter (`citizen`, `gc_5plus`, `gc_under18_no5`, `refugee`). KS has no state-funded food program for immigrants (unlike WA's FAP), so a green card holder under five years who is 18 or older sees no food program at all.
    - **Suggestion — no screener improvement:** Because citizenship isn't a screener input at all (it's handled only as a results-page display filter), there's no screener field to add. The single action is keeping that filter accurate: `non_citizen` and `otherWithWorkPermission` stay off the program, `refugee` stays on it, and bare `gc_5less` is replaced by `gc_under18_no5` so the under-18 exemption to the five-year bar is expressed without claiming the barred adults. An earlier revision of this spec read the July 2025 federal changes as removing refugees and asylees from SNAP entirely; the governing exemption table treats refugee/asylee as exempt from the bar, and that reading now applies. The Cuban/Haitian/COFA edge stays an accepted inclusivity assumption (small group; PE's COFA gap #8296 mishandles it anyway), and Cuban/Haitian entrant has no filter option at all.
...
- **`legal_status_required`** → `citizen`, `gc_5plus`, `gc_under18_no5`, `refugee`. `non_citizen` and `otherWithWorkPermission` are not SNAP-eligible. `refugee` is exempt from the five-year bar and stays on the program — this supersedes an earlier revision of this spec, which dropped it on a post-OBBBA reading. Bare `gc_5less` is replaced by `gc_under18_no5`: SNAP exempts green card holders under 18 from the bar regardless of the status they adjusted from, while the barred adults have no KS program to fall back to.
