...
class TestMoTanfWiring(TestCase):
    ...
    def test_pe_inputs_includes_ssi_aware_cash_assets(self):
        """The $1,000 resource test reads the reported assets, except where an SSI
        recipient's excluded share cannot be isolated from the household total."""
        self.assertIn(dependency.spm.CashAssetsExcludingSsiHouseholdsDependency, MoTanf.pe_inputs)

    def test_does_not_send_the_plain_aggregate(self):
        """Both would write spm_unit_cash_assets; the SSI-aware one must win."""
