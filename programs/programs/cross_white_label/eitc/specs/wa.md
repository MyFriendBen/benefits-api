...
9. **Qualifying child — Relationship test**
   ...
   - **Suggested screener improvements**: (a) distinguish formal `fosterChild` placements from informal kinship care — note the `was_in_foster_care` tile does **not** resolve this, since it asks whether a person was ever in foster care rather than qualifying an existing `fosterChild` member (see `programs/programs/FOSTER_CARE_SCREENER_GAPS.md`); (b) add a follow-up under `other` — "Is this person your niece, nephew, half-sibling, or another descendant relationship such as great-grandchild?" Cross-program value beyond EITC.
   - Screener fields:
     - `relationship`
