...
#
# Strategy:
#   1. Create any missing WL-scoped ProgramCategory rows (CESN cash/food/
#      transportation/housing, CO efficiency_upgrades). Reuses the existing
#      ProgramCategoryManager.new_program_category() helper so new categories
#      are created the same way the rest of the codebase does it.
#   2. Set the English text on each new category's translations
#      (new_program_category creates blank translation rows).
#   3. Link programs to categories — only where category_id IS NULL, so this
#      migration is idempotent and never clobbers an admin-set category.
...
NEW_CATEGORIES = [
    ...
    ("co", "co_efficiency_upgrades", "Energy Efficiency Upgrades"),
]
...
PROGRAM_CATEGORY_LINKS = [
    ...
    # CO — link to existing categories, plus the new co_efficiency_upgrades
    ("co", "co_andso", "cash"),
    ("co", "co_section_8", "housing"),
    ("co", "co_care", "co_efficiency_upgrades"),
    ...
    # don't need that one here, but flagging the dupe for cleanup
    # (see MFB-???).
