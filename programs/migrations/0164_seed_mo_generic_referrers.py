...
# (e.g. ks). Partner-org referrers are added separately via the Django admin.
MO_GENERIC_REFERRERS = {
    "flyers": "Flyer",
...
def seed_mo_generic_referrers(apps, schema_editor):
    """Create the standard generic Referrer rows for the Missouri (mo) white label.

    Idempotent: skips any (white_label, referrer_code) that already exists so it
    never clobbers operational data. No-ops if the mo WhiteLabel does not exist
    yet (it is created as part of the MO launch); rerun after the WL exists, or
    add rows via the Django admin per the white label README.
    """
    WhiteLabel = apps.get_model("screener", "WhiteLabel")
    db = schema_editor.connection

    white_label = WhiteLabel.objects.filter(code="mo").first()
    if white_label is None:
        return

    for code, name in MO_GENERIC_REFERRERS.items():
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(seed_mo_generic_referrers, reverse_seed),
    ]
