...
from programs.programs.white_labels.mo.ssdi.calculator import MoSsdi
...
def make_calculator(has_ssdi=False):
    ...
    return MoSsdi(mock_screen, mock_program, {}, mock_missing_deps)
...
class TestMoSsdiClassAttributes(TestCase):
    ...
    def test_sga_thresholds_are_2026_figures(self):
        ...
        self.assertEqual(MoSsdi.sga_blind, 2_830)

    def test_member_amount_is_current_ssa_average(self):
        # SSA estimated average monthly benefit for all disabled workers
        # payable January 2026, after the 2.8% COLA.
        self.assertEqual(MoSsdi.member_amount, 1_630 * 12)

    def test_dependencies(self):
        self.assertEqual(MoSsdi.dependencies, ["age", "income_amount", "income_frequency"])


class TestMoSsdiFraSchedule(TestCase):
    def test_born_1954_fra_is_66_0(self):
        self.assertEqual(MoSsdi._get_fra(1954), (66, 0))

    def test_born_1955_fra_is_66_2(self):
        self.assertEqual(MoSsdi._get_fra(1955), (66, 2))

    def test_born_1957_fra_is_66_6(self):
        self.assertEqual(MoSsdi._get_fra(1957), (66, 6))

    def test_born_1960_fra_is_67_0(self):
        self.assertEqual(MoSsdi._get_fra(1960), (67, 0))

    def test_born_1943_fra_is_66_0(self):
        self.assertEqual(MoSsdi._get_fra(1943), (66, 0))

    def test_born_1970_fra_is_67_0(self):
        self.assertEqual(MoSsdi._get_fra(1970), (67, 0))


class TestMoSsdiMemberEligibility(TestCase):
...
class TestMoSsdiHouseholdEligibility(TestCase):
...
class TestMoSsdiValue(TestCase):
    def test_eligible_member_gets_current_ssa_average(self):
        calc = make_calculator()
        member = make_member()
        calc.screen.household_members.all.return_value = [member]
        result = calc.calc()
        self.assertTrue(result.eligible)
        self.assertEqual(result.value, 1630 * 12)
    ...
    def test_two_eligible_members_each_get_value(self):
        calc = make_calculator()
        first = make_member(age=44, birth_year=1981)
        second = make_member(age=39, birth_year=1986)
        calc.screen.household_members.all.return_value = [first, second]
        result = calc.calc()
        self.assertTrue(result.eligible)
        self.assertEqual(result.value, 2 * 1630 * 12)
