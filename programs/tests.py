...
class ProgramNameAbbreviatedNormalizationTests(TestCase):
    ...
    def test_new_program_lowercases_name_abbreviated(self) -> None:
        program = Program.objects.new_program(self.white_label.code, "SNAP")
        program.refresh_from_db()
        self.assertEqual(program.name_abbreviated, "snap")

    def test_save_lowercases_mixed_case_on_update(self) -> None:
        program = Program.objects.new_program(self.white_label.code, "snap")
    ...
    def test_save_leaves_already_lowercase_unchanged(self) -> None:
