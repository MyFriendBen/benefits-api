...
2. **Household income must be at or below the TANF/SFA payment standard for household size (after applying earned-income disregards)**
   ...
   - Cash assistance: the screener offers two types. `cashAssistance` ("Cash Assistance - TANF") is the household's own grant and is excluded from its own recalculation; `cashAssistanceOther` ("Cash Assistance - Other" — General Assistance, another state's TANF, a local fund) is sent to PolicyEngine as `financial_assistance` and counted in full as unearned income, with no disregard.
