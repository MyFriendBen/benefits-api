...
class TestPolicyEngineConfigSingleton(TestCase):
    def test_save_enforces_single_row(self) -> None:
    ...
    def test_current_version_does_not_write_when_absent(self) -> None:
    ...
    def test_current_version_returns_stored_value(self) -> None:
    ...
    def test_clean_rejects_non_version_strings(self) -> None:
    ...
    def test_save_rejects_invalid_via_full_clean(self) -> None:
...
class TestPolicyEngineConfigAdminDisplay(TestCase):
    ...
    def test_version_display_shows_pinned_value(self) -> None:
        config = PolicyEngineConfig(policyengine_version="1.715.2")
        self.assertEqual(self.admin.version_display(config), "1.715.2")

    def test_version_display_shows_default_when_blank(self) -> None:
        config = PolicyEngineConfig(policyengine_version="")
        self.assertEqual(self.admin.version_display(config), PolicyEngineConfigAdmin.DEFAULT_LABEL)

    def test_active_version_display_mirrors_value(self) -> None:
        self.assertEqual(
    ...
    def test_active_version_display_handles_none_on_add_form(self) -> None:
...
class TestResolvePeVersion(TestCase):
    ...
    def test_falls_back_to_config_when_no_override(self) -> None:
        PolicyEngineConfig.objects.create(policyengine_version="1.715.2")
        self.assertEqual(determine_pe_version(None), "1.715.2")

    def test_returns_none_when_unset_and_no_override(self) -> None:
        # No config row, no override => omit the version field.
        self.assertIsNone(determine_pe_version(None))

    def test_blank_config_returns_none(self) -> None:
...
class TestPeInputVersionInjection(TestCase):
    ...
    def test_omits_version_when_unset(self) -> None:
        raw_input = pe_input(self.screen, [])
        self.assertNotIn("version", raw_input)

    def test_injects_config_version(self) -> None:
        PolicyEngineConfig.objects.create(policyengine_version="1.715.2")
        raw_input = pe_input(self.screen, [])
        self.assertEqual(raw_input["version"], "1.715.2")

    def test_override_injects_version(self) -> None:
