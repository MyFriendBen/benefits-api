...
13. **Foster Care, Adoption Support, and Foster Care Alumni: Federal Medicaid categorical pathways; no income test**
    ...
    * **Sub-data-gap (Alumni only) — field shipped, pathway still open in WA.** `household_member.was_in_foster_care` (Step 5, "Ever in foster care, even briefly") now captures the foster-care history fact, and PolicyEngine's `medicaid_work_requirement_eligible` applies the age window itself (`was_in_foster_care & (age < former_foster_care_age_limit)`), so no age logic is needed here. ⚠️ **But the value never reaches PolicyEngine in WA.** WA Medicaid does not declare `FosterCareDependency`, and neither does anything else in the WA white label: `FosterCareDependency` is declared only by the PolicyEngine `HeadStart`/`EarlyHeadStart` bases, and `WaHeadStart` is a custom `ProgramCalculator` with no `pe_inputs`. Verified against a WA screen with the tile ticked on a 22-year-old — `was_in_foster_care` is absent from the payload. The field reaches PolicyEngine only in `il`, `ks`, `ma`, `mo` and `tx` (and in those, only as a side effect of the shared per-screen payload). ⚠️ **And PolicyEngine's Medicaid would not read it even if delivered.** `medicaid_work_requirement_eligible` is a work-*requirement exemption*, not the categorical alumni pathway described above; it is in no calculator's `pe_outputs` and appears nowhere in this codebase. Measured on a KS screen where the field *is* delivered: `medicaid` stayed `0.0`, `medicaid_category` stayed `"NONE"`. PolicyEngine's `medicaid_category` enum has no foster-care value. So declaring `FosterCareDependency` on the Medicaid `pe_inputs` is necessary but not sufficient here — closing this needs a PolicyEngine foster category or a custom WA pathway. Retain the description surfacing as the real backstop. See `programs/programs/FOSTER_CARE_SCREENER_GAPS.md`. Retain the alumni pathway in the program description as a backstop for users who don't tick the tile, and note the tile does not capture whether the user was *on Medicaid* at 18, which the federal rule also requires.
    * 💡 **Screener improvement — adoption support:** (see below) — the alumni improvement is shipped.
...
| 6 | ~~Add `was_in_foster_care` (Yes/No) checkbox (Special Circumstances)~~ **shipped** — PolicyEngine applies the 18–26 window itself | Criterion 13 sub-gap — Foster Care Alumni identification | Chafee, ETV, state tuition waivers |
