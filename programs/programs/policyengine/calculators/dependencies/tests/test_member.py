...
class TestWicEnrolledDependency(TestCase):
    """Tests for WicEnrolled, which reports WIC enrollment for adjunctive eligibility (e.g. TX FPP §4140)."""
    ...
    def test_field_name(self):
        dep = member.WicEnrolled(self.screen, self.head, {})
        self.assertEqual(dep.field, "wic")
    ...
    def test_value_returns_none_when_not_enrolled(self):
        self.screen.has_wic = False
        self.screen.save()
        dep = member.WicEnrolled(self.screen, self.head, {})
        self.assertIsNone(dep.value())


class TestChipEnrolledDependency(TestCase):
    """Tests for ChipEnrolled, which reports CHIP enrollment for adjunctive eligibility (e.g. TX FPP §4140)."""
    ...
    def test_field_name(self):
        dep = member.ChipEnrolled(self.screen, self.head, {})
        self.assertEqual(dep.field, "chip")

    def test_value_returns_one_when_enrolled(self):
        """CHIP is household-level (§4140 covers the applicant or their child)."""
        self.screen.has_chp = True
        self.screen.save()
        dep = member.ChipEnrolled(self.screen, self.head, {})
        self.assertEqual(dep.value(), 1)

    def test_value_returns_none_when_not_enrolled(self):
        self.screen.has_chp = False
        self.screen.save()
        dep = member.ChipEnrolled(self.screen, self.head, {})
        self.assertIsNone(dep.value())
