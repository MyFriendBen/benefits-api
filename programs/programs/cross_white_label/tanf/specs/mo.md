...
| SSI/SP/SAB member's resources | Excluded from countable resources | ⚠️ Not applied — `household_assets` is a single aggregate with no per-member breakdown, and the input is shared with every other program on the screen, so the excluded member's share is counted against the limit (Criterion 7, MFB-1696) |
...
- **⚠️ Accepted limitation — SSI/SP/SAB member present:** their resources are excluded by rule, but the exclusion is not applied. PolicyEngine's `mo_tanf_countable_resources` reads person-level asset ownership the screener doesn't collect, and `spm_unit_cash_assets` is shared with every other program on the screen — so the reported total is counted in full and such a household can be denied on it. Tracked in MFB-1696.
